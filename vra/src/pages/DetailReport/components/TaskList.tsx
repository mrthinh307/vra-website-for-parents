...
const TaskList: React.FC<TaskListProps> = ({ 
  ...
  // Fetch tasks when component mounts or lessonProgressId changes
  useEffect(() => {
    const fetchTasks = async () => {
      ...
      try {
        ...
        const taskData = await missionService.getTasks(lessonProgressId);
        setTasks(taskData);
      } catch (err) {
    ...
    <div className="bg-white rounded-lg shadow-lg overflow-hidden border border-gray-100 mb-8">
      ...
      {loading && (
        <div className="flex justify-center items-center p-8">
          <Loader2 className="animate-spin mr-2 text-primary-color" />
          <span className="text-gray-600">Đang tải dữ liệu nhiệm vụ...</span>
        </div>
      )}
          ...
          <div className="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
            <div>
              <p className="text-sm text-gray-700">
                Hiển thị <span className="font-medium">1</span> đến <span className="font-medium">{tasks.length}</span> trong số <span className="font-medium">{tasks.length}</span> kết quả
              </p>
            </div>
